...
import { TavoriEventSchema } from "../types/drive.js";
...
export interface EventServerConfig {
  ...
  eventsDir?: string; // default: ~/.tavori/events/
}
...
export class EventServer {
  ...
  constructor(driveSystem: DriveSystem, config?: EventServerConfig, logger?: Logger) {
    ...
    // Default events directory: ~/.tavori/events/
   ...
   * When a file appears:
   *   1. Read and parse it
   *   2. Validate via TavoriEventSchema
  ...
  private handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
    ...
    req.on("end", () => {
      try {
        ...
        const event = TavoriEventSchema.parse(data);
