...
export class Logger {
  ...
  constructor(config: LoggerConfig) {
    ...
    this.currentFile = path.join(this.dir, "tavori.log");
    ...
    const doRotate = async (): Promise<void> => {
      try {
        if (rotation.type === "date") {
          const dest = path.join(this.dir, `tavori.${rotation.date}.log`);
          ...
          for (let i = this.maxFiles - 1; i >= 1; i--) {
            const older = path.join(this.dir, `tavori.${i + 1}.log`);
            const newer = path.join(this.dir, `tavori.${i}.log`);
          ...
          await fsp.rename(this.currentFile, path.join(this.dir, "tavori.1.log")).catch(() => {
            // File may not exist — ignore
          });
        }
      } catch (err) {
