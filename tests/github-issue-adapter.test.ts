...
describe("GitHubIssueAdapter.parsePrompt", () => {
  ...
  it("deduplicates labels when JSON and defaults overlap", () => {
    const adapter2 = new GitHubIssueAdapter({ defaultLabels: ["tavori"] });
    const prompt = "```github-issue\n{\"title\":\"T\",\"body\":\"B\",\"labels\":[\"tavori\"]}\n```";
    const result = adapter2.parsePrompt(prompt);

    const tavoriCount = result.labels.filter((l) => l === "tavori").length;
    expect(tavoriCount).toBe(1);
  });
});
...
describe("GitHubIssueAdapter.execute", () => {
  ...
  afterEach(() => {
    vi.useRealTimers();
    delete process.env["TAVORI_GITHUB_REPO"];
  });
  ...
  it("uses TAVORI_GITHUB_REPO env var when config.repo is empty", async () => {
    process.env["TAVORI_GITHUB_REPO"] = "env-owner/env-repo";
    const adapter = new GitHubIssueAdapter();
    const dedupChild = makeDedupChild();
    const child = makeFakeChild();
  ...
  it("config.repo takes priority over TAVORI_GITHUB_REPO env var", async () => {
    process.env["TAVORI_GITHUB_REPO"] = "env-owner/env-repo";
  ...
  it("passes labels as multiple --label args to gh", async () => {
    const adapter = new GitHubIssueAdapter({
      repo: "owner/repo",
      defaultLabels: ["tavori", "automated"],
    });
    const dedupChild = makeDedupChild();
    const child = makeFakeChild();
