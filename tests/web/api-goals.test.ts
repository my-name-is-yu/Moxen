...
// Mock tavori-client with a factory so we can swap the impl per test
let mockSm: {
...
vi.mock('../../web/src/lib/tavori-client', () => ({
...
describe('GET /api/goals', () => {
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tavori-web-goals-'));
    mockSm = {
...
describe('GET /api/goals/:id/tasks', () => {
  ...
  it('returns tasks from JSON files sorted by created_at desc', async () => {
    const tasksDir = path.join(tmpDir, '.tavori', 'tasks', 'goal-1');
