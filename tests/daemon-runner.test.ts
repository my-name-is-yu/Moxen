...
describe("DaemonRunner", () => {
  ...
  describe("generateCronEntry()", () => {
    ...
    it("should use 60-minute default when no interval is provided", () => {
      ...
      expect(entry).toBe("0 */1 * * * /usr/bin/env tavori run --goal goal-default");
    });
    ...
    it("should treat interval <= 0 as 60 minutes", () => {
      ...
      expect(entry0).toBe("0 */1 * * * /usr/bin/env tavori run --goal goal-x");
      expect(entryNeg).toBe("0 */1 * * * /usr/bin/env tavori run --goal goal-x");
    });
