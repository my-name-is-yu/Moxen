...
describe("isRewardLogEnabled", () => {
  beforeEach(() => {
    delete process.env["MOTIVA_REWARD_LOG"];
  });
  ...
  it("returns true when MOTIVA_REWARD_LOG=1", () => {
    process.env["MOTIVA_REWARD_LOG"] = "1";
    expect(isRewardLogEnabled()).toBe(true);
  });

  it("returns false when MOTIVA_REWARD_LOG=0", () => {
    process.env["MOTIVA_REWARD_LOG"] = "0";
    expect(isRewardLogEnabled()).toBe(false);
  });

  it("returns false when MOTIVA_REWARD_LOG=true (not exactly '1')", () => {
    process.env["MOTIVA_REWARD_LOG"] = "true";
    expect(isRewardLogEnabled()).toBe(false);
  });
});
