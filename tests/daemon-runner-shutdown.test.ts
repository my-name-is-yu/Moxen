...
function makeTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "tavori-shutdown-test-"));
}
...
describe("DaemonRunner — Graceful Shutdown + Crash Recovery", () => {
  ...
  describe("rotateLog()", () => {
    it("should not rotate when log file is below the size threshold", async () => {
      ...
      // tavori.log should still exist (not rotated)
      expect(fs.existsSync(logPath)).toBe(true);
      // No rotated files should exist
      const entries = fs.readdirSync(logDir);
      const rotated = entries.filter((f) => f !== "tavori.log");
    ...
    it("should rotate when log file exceeds the size threshold", async () => {
      ...
      // tavori.log should no longer exist (was renamed)
      expect(fs.existsSync(logPath)).toBe(false);
      // At least one rotated file should exist
      const entries = fs.readdirSync(logDir);
      const rotated = entries.filter((f) => /^tavori\..+\.log$/.test(f) && f !== "tavori.log");
    ...
    it("should keep at most maxFiles rotated log files", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        fs.writeFileSync(path.join(logDir, `tavori.${ts}.log`), "old log", "utf-8");
    ...
    it("should name rotated file with a timestamp suffix", async () => {
      ...
      // Name should match tavori.<timestamp>.log pattern
      expect(rotated[0]).toMatch(/^tavori\..+\.log$/);
    });
  });
});
