...
- `src/cli-runner.ts` extended — `tavori suggest`, `tavori improve [path]` commands
- Status: complete
...
- `src/runtime/plugin-loader.ts` — dynamic plugin loading from `~/.tavori/plugins/`, manifest validation, auto-registration into AdapterRegistry/DataSourceRegistry/NotifierRegistry
...
- `src/runtime/event-server.ts` extended — real-time file watcher for `~/.tavori/events/` (fs.watch)
