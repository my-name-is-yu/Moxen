...
- `src/cli-runner.ts` extended — `motiva suggest`, `motiva improve [path]` commands
- Status: complete
...
- `src/runtime/plugin-loader.ts` — dynamic plugin loading from `~/.motiva/plugins/`, manifest validation, auto-registration into AdapterRegistry/DataSourceRegistry/NotifierRegistry
...
- `src/runtime/event-server.ts` extended — real-time file watcher for `~/.motiva/events/` (fs.watch)
