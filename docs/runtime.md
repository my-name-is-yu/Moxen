...
```
CLIラッパー  ─┐
デーモン     ─┤→  コアループ（observe→gap→score→task→execute→verify）
cronラッパー ─┘
```
...
**経験ログ**: 状態 → 行動 → 結果の記録。学習の基盤となるデータだ。個々のセッションでは参照されないが、戦略選択やタスク生成の品質を時間とともに向上させる。これがMotivaの「長期記憶」だ。
