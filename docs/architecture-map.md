...
| `runtime.md` | ランタイム基盤。オーケストレーション・駆動・コンテキスト | mechanism.md | drive-system, session-and-context, execution-boundary | **P1未解決: プロセスモデル未決定** |

### 設計ドキュメント（design/）

| ドキュメント | コンポーネント | 読み込み元 | 出力先 | ステータス |
|---|---|---|---|---|
| `state-vector.md` | 状態ベクトル（多次元のゴール状態表現） | mechanism.md, observation.md | gap-calculation.md, satisficing.md | **P1未解決: confidence調整3重問題** |
| `observation.md` | 観測システム（3層観測、進捗上限ルール） | runtime.md | state-vector.md | **P1未解決: observation_methodスキーマ未定義** |
| `gap-calculation.md` | ギャップ計算（正規化、信頼度加重） | state-vector.md | drive-scoring.md, stall-detection.md | 設計済み |
| `drive-scoring.md` | 駆動スコアリング（3駆動力の数値化） | gap-calculation.md | task-lifecycle.md, drive-system.md | **P1未解決: opportunity_value入力元未定義** |
| `task-lifecycle.md` | タスクライフサイクル（生成→実行→検証→失敗対応） | drive-scoring.md, execution-boundary.md | state-vector.md (結果反映) | **P1未解決: estimated_duration不在、revert失敗処理未定義** |
| `goal-negotiation.md` | ゴール交渉（実現可能性評価、カウンター提案） | mechanism.md | state-vector.md (初期設定) | 設計済み |
| `satisficing.md` | 満足化（完了判断、プログレス上限） | state-vector.md, observation.md | curiosity.md (タスクキュー空) | 設計済み |
| `stall-detection.md` | 停滞検知（4指標、段階的対応） | gap-calculation.md, task-lifecycle.md | drive-scoring.md (decay_factor), goal-negotiation.md (再交渉) | **P1未解決: 停滞タイプ→原因分類マッピング不在** |
| `trust-and-safety.md` | 信頼と安全（2軸マトリクス、不可逆ルール） | mechanism.md | task-lifecycle.md (実行許可判定) | **P1未解決: 信頼スコア数値閾値未定義** |
| `curiosity.md` | 好奇心（メタ動機、新ゴール提案） | satisficing.md, stall-detection.md | goal-negotiation.md (新ゴール) | **P1未解決: クロスゴール類似度計算未定義** |
| `session-and-context.md` | セッション/コンテキスト管理（3階層記憶） | runtime.md | task-lifecycle.md (コンテキスト組立) | **P1未解決: コンテキスト選択アルゴリズム不在** |
| `drive-system.md` | 駆動方式（4トリガー、起動判断） | runtime.md, drive-scoring.md | コアループ起動 | **P1未解決: イベント駆動トリガー受信口未設計** |
...
| ドキュメント | 未解決事項 | 影響範囲 |
|---|---|---|
| `state-vector.md` | confidence調整3重問題（観測信頼度・有効達成度・進捗上限で3回使われる） | ギャップ計算・満足化判断 |
| `observation.md` | observation_methodのスキーマが未定義（JSONスキーマ・バリデーション） | 観測の実装全体 |
| `drive-scoring.md` | opportunity_valueの入力元が未定義（誰が・いつ・どう検知するか） | 機会駆動スコアリング |
| `trust-and-safety.md` | 信頼スコアの数値閾値が未定義（どの値で象限が切り替わるか） | 自律実行判定 |
| `runtime.md` | プロセスモデル未決定（デーモン/cron/ハートビート/どれを使うか） | 駆動システム全体 |
| `session-and-context.md` | コンテキスト選択アルゴリズム不在（何をロードし何を省くかのロジック） | 全セッション起動 |
| `drive-system.md` | イベント駆動トリガーの受信口が未設計（webhook? polling? pub/sub?） | イベント起動 |
| `stall-detection.md` | 停滞タイプ→原因分類のマッピングロジック不在 | 停滞対応の自動化 |
| `task-lifecycle.md` | estimated_duration不在・revert失敗時の処理が未定義 | 時間超過検知・discard処理 |
| `curiosity.md` | クロスゴール類似度計算の具体的アルゴリズム未定義 | ゴール間の戦略転用 |
...
**ゴール交渉（goal-negotiation）**
5ステップ（受取→次元分解→ベースライン観測→実現可能性評価→応答）。ハイブリッド評価（定量+定性）。3種の応答（受諾/カウンター提案/要注意フラグ）。停滞検知や前提変化で再交渉が発生。
