...
| AIエージェント（Claude Code, Devin） | 複雑なタスクを自律的に実行する | タスク単位。長期目標の追求、戦略の立案と修正はしない |
...
ゴールを追うために、Motiva はあらゆるエージェントを活用する。AIエージェントへの指示、API呼び出しの委譲、コード実行の依頼、外部サービス連携の設定。これらを Motiva が直接行うことはない。Claude Code に実装を指示し、別のエージェントにレビューを依頼し、適切なシステムにデプロイを委譲する——これらは全て、Motiva がオーケストレートするものであり、Motiva 自身が実行するものではない。
