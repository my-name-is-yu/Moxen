...
> Tavori does not chase perfection. It judges "good enough" and stops.
...
Tavori solves this problem structurally. **Stop when the threshold is exceeded.** That is the rule.
...
In Tavori, when a goal is set, the advisor consults with the user to set thresholds. These thresholds become the definition of "good enough."
...
Even when multiple Gaps exist, Tavori selects a **manageable subset** to address.
...
Tavori manages multiple dimensions **each with independent thresholds**.
...
The user can change thresholds at any time. However, Tavori makes the impact of the change explicit.
...
### Tavori's Adjustment Proposals

If Tavori judges from observation that "this threshold is unrealistic" or "it's actually too low," it can propose an adjustment to the user.
