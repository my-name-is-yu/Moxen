...
Tavori's goal negotiation evaluates "is this achievable?" But "is this achievable?" and "should this be done?" are different questions.
...
`trust-and-safety.md` designs safety around **how Tavori behaves**. A low trust balance means it acts cautiously. Low confidence means it seeks confirmation. Irreversible operations require human approval. These are constraints on Tavori's **behavior**.

The ethics gate designs safety around **what Tavori pursues**. It judges whether goals themselves, sub-goals, and the means used in tasks are ethically and legally acceptable.
...
This is not a personality setting — it is a **structural constraint**. Not "Tavori tends to behave ethically," but "goals that do not pass the ethics gate will not be executed." This structural guarantee is on par with, or stronger than, the irreversible action rule, which guarantees "no execution without approval, no matter how high the confidence."
...
Reason: This goal constitutes unauthorized access and privacy violation.
        Tavori will not begin pursuing this goal.
```

**User override: Not permitted.** Even if the user says "do it anyway," Tavori will not comply. The same logic applies here as in `trust-and-safety.md` §4, where "the final filter of human approval cannot be removed regardless of how high the score is" — in the ethics gate, user instructions do not replace that filter.
...
**Why record passes too**: To allow retroactive verification of "why this goal was executed." The transparency of goal selection is the foundation of Tavori's trustworthiness.
...
```json
{
  ...
  "rejection_delivered": {
    "message": "This sub-goal constitutes a privacy violation. Tavori will not begin pursuing it.",
...
```
tavori ethics-log            # Show all logs
tavori ethics-log --rejected  # Show rejections only
tavori ethics-log --goal <id> # Show verdict history for a specific goal
```
...
`trust-and-safety.md` §4 states: "Tavori's judgment is probabilistic. A confidence of 0.95 means 'there is a 5% chance of being wrong.' A 5% error rate in irreversible actions is unacceptable."

The same logic applies to the ethics gate's "Clear No." Even if the user says "this is okay," Tavori's ethical judgment is not overridden. The user themselves may be intentionally requesting illegal or unethical behavior, and in that case their instruction cannot be treated as the final authority.

Tavori is the user's advisor. But there are instructions an advisor should not follow. Between user autonomy and Tavori's structural constraints, the ethics gate prioritizes the latter.

**Gray zone is not an override**: User confirmation in the gray zone is not changing Tavori's judgment — it is a process of proceeding with acknowledged risk based on additional information. When the user says "I understand and want to proceed," this is an autonomous decision that does not undermine the purpose of the ethics gate.
...
- `execution-boundary.md` — Tavori's execution boundary (consistent with the delegation model)
