...
> The decision structure for when and why Tavori acts. Design of scheduling and triggers.

---

## 1. The Central Question

The Tavori drive system reduces to a single question.
...
There are four types of triggers that activate Tavori. Each has a different timing and a different level of urgency.
...
When a task completes, Tavori automatically executes the following sequence.
...
Event-driven triggering requires a mechanism to receive events. If Tavori cannot receive events, event-driven triggering does not function. The reception mechanism is designed in stages according to the phase of the process model.
...
- `type`: `"external"` (external event) or `"internal"` (Tavori internal trigger)
...
The **`~/.tavori/events/`** directory is used as an event queue. External systems write JSON files to this directory. When `tavori run` is executed, Tavori reads this directory, processes the events, and archives them.
...
Processing flow:
1. On `tavori run` launch, read `~/.tavori/events/`
...
> **Integration with the observation cycle**: Writes to this event queue function as triggers for event-driven observation (Event-driven) defined in `observation.md` §3. Upon receiving an event, Tavori immediately launches the observation cycle for the corresponding goal and re-evaluates the state vector.

**Polling fallback**: External data sources without push notification (APIs, web pages, etc.) are checked directly by Tavori at the configured URL/API during the observation step. This is implemented as an integration into the observation step, not as event-driven.
...
**File watcher**: Watches the `~/.tavori/events/` directory and begins processing the instant a new file is written. Faster response than the MVP polling approach. Maintains file format compatibility with the MVP.
...
Tavori has two operating states.

### Active State

A task is executing, or the task discovery loop is evaluating. Tavori is doing something.
...
No immediate action is needed. Tavori is monitoring but not acting.

There are three patterns of waiting.

**Waiting for effect**: Immediately after a measure was taken. Changes take time to materialize. Tavori decides when to measure and waits. In the meantime, it either turns attention to other goals or waits quietly.

**Waiting on external dependency**: Someone else's approval, a response from an external service, a market reaction. There are timings that Tavori cannot control. Knowing when to wait is the same as not taking unnecessary action.
...
When Tavori is pursuing multiple goals, each goal has its own drive rhythm.
...
Tavori records "why this activation occurred." When multiple triggers converge, all of them are retained as context and passed to the task discovery loop. A compound situation — "the deadline is approaching AND a sensor alert came in" — may be treated with higher priority than a single trigger.
