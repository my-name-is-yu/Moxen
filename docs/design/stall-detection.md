...
Tavori's loop never stops. It keeps running until a goal is achieved or the user intervenes. This "never stopping" quality is normally a strength. Even if one loop iteration is imperfect, the next corrects it.
...
Stall detection is a **circuit breaker**. It detects when the loop is spinning meaninglessly and triggers an intervention. Without stall detection, Tavori would keep making the same moves forever in front of an unsolvable problem.
...
**Diagnosis**: The task requirements exceed the tools, permissions, or knowledge currently available to Tavori. No matter what is tried, the task cannot be executed in the first place.
...
A capability limitation is not a failure. It is evidence that Tavori accurately recognizes its own boundaries. Escalation should be concrete. Not "I can't do this" but "I can do this if I have that."

### 3.4 External Dependency

**Diagnosis**: The task's preconditions are waiting on an external change. Tavori cannot make progress no matter what it does.
...
Without detecting external dependencies, Tavori would spin in an empty loop forever, expecting "the API might support it eventually."
...
This escalation is not a failure report. It is an active proposal by Tavori — recognizing its limits and inviting the user into a more productive conversation.
...
The purpose of graduated responses is to distinguish temporary stalls (plateaus) from genuine ones. New initiatives take time to show results. Judging "it's a stall" without waiting for that time would cause Tavori to abandon a strategy that is actually working.
...
**Why this is necessary**: Leaving a stalled dimension at high priority causes Tavori to keep hitting the same wall. The `decay_factor` temporarily redirects attention to other dimensions. When the stall is resolved, the `decay_factor` returns to normal.
...
**User notification**: Stall detection itself runs silently. The 1st and 2nd detections are handled autonomously by Tavori. Only the 3rd detection (escalation) triggers a user notification. This prevents users from being notified every time a temporary stall occurs.
