...
Motivaのループは止まらない。ゴールが達成されるかユーザーが止めるまで回り続ける。この「止まらない」という性質は、通常は強みだ。1回のループが不完全でも、次のループで補正される。
...
停滞検知は**サーキットブレーカー**だ。ループが無意味に回り続けることを検出し、介入の引き金を引く。停滞検知がなければ、Motivaは解けない問題の前で永遠に同じ手を打ち続ける。
...
**判断基準**: タスクの要件が、Motivaが現在使えるツール・権限・知識の範囲を超えている。何を試しても、そもそもタスクを実行できない。
...
能力の限界は失敗ではない。Motivaが自分の境界を正確に認識していることの証拠だ。エスカレーションは具体的に行う。「できない」ではなく「これがあればできる」を伝える。

### 3.4 外部依存

**判断基準**: タスクの前提条件が外部の変化待ちになっている。Motivaが何をしても進められない。
...
外部依存を検知できないと、Motivaは無限に「APIが対応するかもしれない」という期待で空ループを回し続ける。
...
このエスカレーションは失敗の報告ではない。Motivaが限界を認識し、より生産的な議論にユーザーを引き込むための能動的な提案だ。
...
**なぜこれが必要か**: 停滞している次元を高い優先度のまま放置すると、Motivaは同じ壁に繰り返しぶつかり続ける。decay_factorにより、一時的に別の次元に注意が向く。停滞が解消されれば、decay_factorは元に戻る。
...
**ユーザーへの通知**: 停滞検知自体はサイレントに動く。第1検知・第2検知はMotivaが自律的に対応する。第3検知（エスカレーション）になって初めてユーザーに通知する。これにより、一時的な停滞のたびにユーザーに通知が届く状況を防ぐ。
