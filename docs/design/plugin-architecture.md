...
> Motivaのプラグインは「ユーザーが使うツール」ではなく「Motivaが自律的に選択・活用するツール」だ。
> 本ドキュメントは、外部サービス連携・通知・データ観測をプラグインとして拡張するための
> 仕組みと、Motivaがそれらをどのように自律的に選択・信頼評価するかを定義する。
...
### Motiva vs Claude Code / OpenClaw

Claude CodeやOpenClawにおけるプラグインは「ユーザーが明示的に呼び出すツール」だ。ユーザーがコマンドを実行し、ツールが応答する。主体はユーザーにある。

Motivaにおけるプラグインは異なる。Motivaのコアループ（観測 → ギャップ → スコア → タスク → 実行 → 検証）はユーザーの指示なしに自律的に動く。したがって、プラグインも**Motivaが自律的に選択し、コアループの中に統合して使いこなす**ものでなければならない。「このプラグインを呼んでください」というユーザー指示を必要としないことが、Motivaのプラグイン設計の出発点だ。
...
Motivaのコアに含めるべきものは最小限だ。以下の基準で判断する。
...
この原則により、Motivaのコアは小さく保たれ、サービス固有のロジックはプラグインに委譲される。

---

## §2 プラグイン種別

Motivaは3種類のプラグインをサポートする。それぞれ既存のインターフェースまたは新規インターフェースに対応する。

### 2.1 adapterプラグイン（IAdapter実装）

**役割**: タスクの実行先。Motivaがタスクを委譲するエージェント・システムを追加する。
...
**役割**: イベント通知の送信先。Motivaが特定のイベントを検知した際に通知を送る。
...
各プラグインはマニフェストファイル（`plugin.yaml` または `plugin.json`）を同梱する。マニフェストは「このプラグインは何ができるか」をMotivaの自律選択エンジンに伝えるための能力宣言だ。

### 3.1 マニフェストの例

```yaml
# ~/.motiva/plugins/jira-source/plugin.yaml
name: jira-source
version: "1.0.0"
type: data_source
capabilities:
...
min_motiva_version: "1.0.0"
```

```yaml
# ~/.motiva/plugins/slack-notifier/plugin.yaml
name: slack-notifier
version: "2.1.0"
type: notifier
capabilities:
...
supported_events:
  ...
  - stall_detected
  - task_blocked
description: "MotivaのイベントをSlackチャンネルに通知する"
config_schema:
  channel:
    type: string
    required: true
    description: "通知先Slackチャンネル（例: #motiva-alerts）"
  mention_on_critical:
    type: boolean
    default: true
...
プラグインは `~/.motiva/plugins/` 以下のサブディレクトリとして配置する。
...
ディスカバリーは起動時に1回実行される。`~/.motiva/plugins/` 内の各サブディレクトリを走査し、`plugin.yaml` または `plugin.json` が存在するものをプラグイン候補とみなす。
...
プラグインのロード失敗はMotiva本体をクラッシュさせない。
...
Motiva起動時の動作:
  ...
  - `motiva plugin list` で各プラグインの状態を確認可能
```

---

## §5 Motivaによるプラグイン自律選択（3フェーズ）

Motivaがプラグインを自律的に活用する能力は、3つのフェーズで段階的に強化される。

### Phase 1 — 手動設定（M9で実装）

ゴール定義の中でプラグインを明示的に指定する。Motivaはその指定に従うだけで、自律的な選択は行わない。
...
Phase 1の自律性はゼロだ。「どのプラグインを使うか」はすべてユーザーが決める。ただし「プラグインをいつ呼ぶか」はMotivaがコアループの中で判断する。
...
type NotificationEventType =
  ...
  | "trust_change";      // プラグインまたはMotiva本体の信頼スコアが大きく変化
```
...
MVPではプラグインはMotiva本体と同一プロセスで動作する。サンドボックスは設けない。この決定の根拠と制約は以下だ。
...
同一プロセス実行の制約として、悪意あるプラグインはMotivaの内部状態に直接アクセスできる。これを許容できるのは「ユーザーが自ら `~/.motiva/plugins/` に配置した」という明示的な信頼行為があるためだ。
...
```
~/.motiva/plugins/<plugin-name>/config.json   # ユーザー設定（APIキー等を含む）
```
...
MVPでは権限宣言は**情報提供目的**であり、実行時の強制はしない。将来フェーズで権限エンフォースメントを追加する際の前提情報として機能する。`shell: true` を宣言するプラグインは `motiva plugin install` 時に明示的な警告を表示する。
...
- CLI: `motiva plugin list`, `motiva plugin install <path>`, `motiva plugin remove <name>`
- Phase 1のゴール定義での `plugin_config` フィールドサポート

**完了基準**:
- `~/.motiva/plugins/` に配置されたプラグインが起動時に自動検出・登録される
- ロード失敗プラグインがMotivaのクラッシュを引き起こさない
...
| フェーズ | 内容 |
|---------|------|
| プラグインマーケットプレイス | `motiva plugin search <keyword>` でコミュニティプラグインを検索・インストール |
| バージョン管理 | `min_motiva_version` / `max_motiva_version` の強制、破壊的変更の移行支援 |
