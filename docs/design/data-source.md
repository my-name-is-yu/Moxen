# Tavori — External Data Source Integration Design

---

## 1. Overview

Tavori's observation system (`observation.md` §2 Layer 1) can retrieve data from files and HTTP APIs as a means of mechanical observation. This document designs the abstraction layer for external data sources.
...
DataSourceConfig {
  ...
  auth?: {
    ...
    secret_ref?: string             // Key name in ~/.tavori/secrets/<source_id>.json
  }
...
Secret information is not included in the source configuration file. It is stored separately in `~/.tavori/secrets/<source_id>.json`.

```
~/.tavori/secrets/fitbit_steps.json
{
...
Persistence location: `~/.tavori/data-sources.json`
...
```
tavori datasource add    # Interactive configuration (with connection test)
tavori datasource list   # List registered sources
tavori datasource remove <id>
```
...
Per Tavori's execution boundary principle (`execution-boundary.md` §1), Tavori only observes and judges. Writing to external services is executed as a delegated task to agents.
