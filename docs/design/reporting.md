...
> Tavori reports to the user proactively — without being asked. It delivers the status of goal pursuit at the right time and at the right level of detail.
...
Tavori autonomously pursues user goals. But being autonomous does not mean operating in silence. As defined in vision.md, "Tavori reports proactively, at the right time, at the right level of detail" — this is the essential behavior of an autonomous partner.

Reporting is a core function of Tavori, not an optional one. Whether the user can trust Tavori depends on transparency (see trust-and-safety.md §8). Trust is built on transparency, and reporting is the implementation of transparency.

### Reporting and the Execution Boundary

Generating reports (deciding on content and structure) is done directly by Tavori. This is part of Tavori's reasoning process and corresponds to LLM calls for analyzing and summarizing goal state.

Delivering reports (writing to files, sending notifications) is delegated (see execution-boundary.md §3). Tavori decides "what to report," and "how to deliver it" is left to existing systems.
...
**Purpose**: Regularly convey the overall picture of goal pursuit. This is the foundational information for users to get a bird's-eye view of Tavori's activity.
...
| Next actions | Tasks Tavori plans to work on in the next period |
...
| Approval request | Before executing an irreversible action (see trust-and-safety.md §4) | Action content, trust/confidence levels, Tavori's assessment, approval button |
...
**Purpose**: When Tavori changes strategy, convey the rationale for that decision to the user.

Strategy changes happen autonomously, but "explaining why" is directly tied to user trust. Tavori does not pivot silently.
...
Triggered by Tavori's internal decisions. Not by external input, but fired when a strategy change is determined in the strategy selection step of the core loop (`mechanism.md §2.3`).
...
|-------------|-------------------|
| 1st detection | No report (Tavori handles autonomously) |
...
When running `tavori run`, unread reports and unprocessed notifications are displayed in the console.
...
[Report] Daily Summary (2026-03-10)
  ...
  Details: ~/.tavori/reports/daily/2026-03-10.md

[Notification] Strategy Change (2026-03-10 14:30)
  Goal "Revenue 2x": Pivoted from "UI improvements" → "Support enhancement"
  Details: ~/.tavori/reports/notifications/20260310-143022-strategy-change.md
```

### 5.2 Phase 2: External Notification Channels

Phase 2 adds push notifications. Information reaches the user without them having to check in on Tavori.
...
| Triggers | Time-based (evaluated when `tavori run` is executed), threshold-based, event-based |
...
| Report storage | Saved as Markdown files in `~/.tavori/reports/` |
| Unread management | File-based (unread reports displayed when `tavori run` is executed) |

MVP constraint: Report delivery is pull-only (only when `tavori run` is executed). There are no real-time push notifications. Even urgent alerts are displayed the next time `tavori run` is executed. This is a constraint of the MVP process model (CLI-based), not of the reporting design itself.
...
**Too many vs. too few reports**: Too many reports and users start ignoring them. Too few and users lose sight of what Tavori is doing. The default configuration uses a conservative setup — "daily summary + immediate notifications for urgent events" — with users able to increase or decrease as needed.
...
**Report persistence**: Reports are also part of Tavori's activity log. "When and what was reported" is the foundation of Tavori's transparency and auditability. Therefore, report files are archived rather than deleted.
