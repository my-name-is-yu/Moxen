...
> runtime.md が「結果検証は3層で行われる」と定義する。本ドキュメントはその3層の観測システムを具体的に設計する。観測システムは、Motivaが「世界の今を知る」ための唯一の窓口だ。
...
機械的観測は自動実行される。人間の介在や判断は不要だ。観測手段（どのコマンドを実行するか、どのAPIを叩くか）は Advisor がゴール設定時に定義し、観測サイクルが来るたびに Motiva が自律的に実行する。
...
自己申告の内容は状態ベクトルの `current_value` に直接反映されない。観測ログに記録され、Layer 1・2の観測結果と合わせて Motiva の判断材料になるが、優先度は常に最低だ。
...
イベント駆動の観測は、`drive-system.md` §3 のイベント受信機構（MVP: ファイルキュー `~/.motiva/events/`）を通じてトリガーされる。
...
信頼度が低い観測だけで達成度が閾値に達した次元には、Motiva が自動的に**検証タスク**を生成する。
...
- 繰り返し矛盾が発生する場合、Motiva はエスカレーションを検討する
