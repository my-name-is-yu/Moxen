...
> `runtime.md` defines that "result verification is performed across 3 layers." This document provides a concrete design for that 3-layer observation system. The observation system is Tavori's only window into the current state of the world.
...
Mechanical observations run automatically. No human intervention or judgment is required. The observation method (which command to run, which API to call) is defined by the Advisor at goal-setting time, and Tavori executes it autonomously on each observation cycle.
...
The content of self-reports is not directly reflected in `current_value` in the state vector. It is recorded in the observation log and feeds into Tavori's decision-making alongside Layer 1 and 2 results, but always has the lowest priority.
...
Event-driven observation is triggered via the event reception mechanism in `drive-system.md` §3 (MVP: file queue at `~/.tavori/events/`).
...
For dimensions where achievement has reached the threshold based only on low-confidence observation, Tavori automatically generates a **verification task**.
...
3. Log the unobservable state and notify Tavori
...
- Notify Tavori of the contradiction and generate a resolution task
...
- If contradictions recur repeatedly, Tavori considers escalation
