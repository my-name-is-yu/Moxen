# Tavori品質引き上げロードマップ — roadmapが期待するクオリティへ
...
- **CoreLoopが「完了」を即座に宣言する**: `runOneIteration()` の Step 5（Completion Check、L620-638）がStep 7（Task Cycle、L838）より前にある。全次元が閾値以上（たとえギリギリでも）なら `judgment.is_complete = true` → `return result` でタスク生成に到達しない。つまり**Tavoriは何もしない**
...
tavori goal add "Create a file called /tmp/tavori-test-output.txt with 'hello world'"
tavori run --goal <id> --yes --max-iterations 3
...
```bash
# README品質ゴールを作成
tavori goal add "Improve README.md quality: add installation guide, usage examples, API reference"
tavori run --goal <id> --yes --max-iterations 1
...
- テストシナリオ: 「`/tmp/tavori-test/` に `hello.txt` を作成する」タスクを実際のアダプタで実行
...
手動テスト（最も重要な検証）:
```bash
# Tavoriに「LICENSEファイルを追加する」ゴールを与える
tavori goal add "Add a LICENSE file (MIT) to the project root"
tavori run --goal <id> --yes --max-iterations 3
...
**目的**: `npm install -g tavori && tavori run` が動くこと。
...
**成功基準**: `tavori --yes run --goal <id>` と `tavori run --goal <id> --yes` の両方が同じ動作をする
...
**成功基準**: `tavori datasource add file_existence --path ./README.md` が成功する
...
- `tavori run` 実行中のイテレーション進捗表示（`[Iteration 1/10] observing... gap=0.35, generating task...`）
...
- 未設定の場合は設定方法を含むエラーメッセージを表示:
  ...
  Or configure in ~/.tavori/provider.json
  ```

**ファイル**: `src/provider-factory.ts`（L31-83）

**成功基準**: APIキー未設定で `tavori run` を実行すると、即座に分かりやすいエラーが出る
...
npm link
tavori --help                    # ヘルプ表示
tavori goal add "test goal"      # ゴール作成確認メッセージ
tavori goal list                 # ゴール一覧表示
tavori datasource add file_existence --path ./README.md  # 成功
tavori --yes run --goal <id>     # フラグ位置非依存
```
...
**目的**: R1-R4の修正後、Tavoriが実際にゴールを自律的に達成すること。
