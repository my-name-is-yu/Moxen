...
Tavori — AI agent orchestrator that gives existing agents the drive to persist. Tavori sits above agents and drives them: selecting goals, spawning agent sessions, observing results, and judging completion. Tavori doesn't think — it makes agents think.
...
- Tavori calls LLMs (for goal decomposition, observation) — it is the caller, not the callee
- Execution boundary: Tavori always delegates. Direct actions are LLM calls (for thinking) and state read/write only
...
- State persistence: file-based JSON (~/.tavori/)
- Test: vitest
...
- Layer 15: PluginLoader, NotifierRegistry, INotifier (plugin architecture — dynamic load from `~/.tavori/plugins/`, notifier routing)

## Design Documents

- `docs/vision.md` — why Tavori exists
...
- **Dogfooding推奨モデル**: gpt-5.3-codex（gpt-4o-miniより観測精度・収束速度が大幅に優れる。`~/.tavori/provider.json`で設定）
